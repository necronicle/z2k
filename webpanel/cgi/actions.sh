...
#
# ЧТО ЭТО. z2k-detect classify ЗАМЕРЯЕТ, чем именно режут домен, и выводит
# готовую строку параметров. Не перебор заранее заготовленных плеч: инструмент
# задаёт коробке несколько вопросов сырыми пакетами и собирает ответ из
# измеренных свойств. Команда СТАТЕЛЕСС — ничего не пишет ни в конфиг, ни в
# списки, ни в состояние ротации. Человек сам решает, что делать со строкой.
#
# ПОЧЕМУ ЗАДАЧЕЙ, А НЕ СИНХРОННО, В ОТЛИЧИЕ ОТ detect_probe_domain ВЫШЕ. Та
# проба укладывается в секунды (замерено: 0,18–3,2 с), эта идёт около минуты:
...
strategy_pick_run() {
    ...
    local out="/tmp/z2k-strategy-pick.$$"
    # GODEBUG — тот же, что у службы: без него Go-бинарники падают на MIPS
    # от асинхронного вытеснения.
    GODEBUG=asyncpreemptoff=1 "$bin" classify -json "${domain}:443" > "$out" 2>/dev/null &
    local pid=$! i=0
    echo "Замеряю $domain. Это занимает около минуты — зондов полтора десятка, каждый повторяется трижды."
    while kill -0 "$pid" 2>/dev/null; do
        i=$((i + 1))
        # Потолок втрое выше ожидаемого: на медленной линии повторы растягиваются.
        [ "$i" -gt 180 ] && { kill -9 "$pid" 2>/dev/null; rm -f "$out"; echo "замер не уложился в три минуты" >&2; return 4; }
        [ $((i % 15)) = 0 ] && echo "  идёт замер, ${i} с"
        sleep 1
    done
    wait "$pid" 2>/dev/null
    [ -s "$out" ] || { rm -f "$out"; echo "замер не дал результата" >&2; return 5; }
    mv -f "$out" "$STRATEGY_PICK_OUT"
