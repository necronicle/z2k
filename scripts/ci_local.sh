...
#   - go test идёт без -race и НЕ прод-тулчейном: тестовые бинарники go1.22
#     вообще не запускаются на свежем macOS (dyld: missing LC_UUID) — это
#     несовместимость тулчейна с ОС, не код. Тесты гоняются новейшим
#     доступным go; vet и все сборки остаются на прод-1.22.12. В CI (linux)
#     всё, включая -race, идёт на 1.22.12;
...
# --- Тулчейн Go: прод собирается СТРОГО go1.22.12 (1.26.x ломает MIPS
# soft-float). Валидировать другим тулчейном можно, но об этом надо знать.
if [ -x "$HOME/go/bin/go1.22.12" ]; then
    GO="$HOME/go/bin/go1.22.12"
elif command -v go >/dev/null 2>&1; then
    GO=go
    printf 'ВНИМАНИЕ: go1.22.12 не найден, использую %s — прод собирается 1.22.12\n' "$(go version)"
else
    GO=""
fi
