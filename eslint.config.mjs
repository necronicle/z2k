...
export default [
  {
    // Фронтенд панели: браузер, ES-модули, БЕЗ сборки — файлы отдаются как
    // есть, панель обязана открываться на роутере без интернета и тулчейна.
    //
    // sourceType: "module" — не косметика, а условие работоспособности
    // no-undef. При обычных <script> все объявления живут в общей области
    // видимости, и правилу пришлось бы скармливать 65 общих имён руками:
    // страховка, ради которой линтер заводился, превратилась бы в список,
    // который сам же и пропускает опечатки. С модулями область пофайловая,
    // и заодно ловятся неиспользуемые импорты.
