...
//
// Девяносто секунд — компромисс с запасом до сторожа: дерево до этого места
// тратит около двадцати секунд, остаётся семьдесят на сторожевой зазор.
// Платим этим временем только на трудных доменах: чистый домен заканчивается
// за полсекунды и сюда не доходит вовсе.
const defaultJointBudget = 90 * time.Second
...
// Замер на роутере 04.09, www.instagram.com: так поиск находил общий приём
// «fake-x7+seqovl-hello», но за 3 м 45 с при потолке задачи в 180 с. С
// потолком в 45 и в 90 секунд он не доходил до находки вовсе и сдавался.
// Один проход с фильтром снимает эту развилку целиком.
