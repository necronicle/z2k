...
именем сайта, который у провайдера в белом списке. Настоящее имя остаётся
настоящим (шифрование не ломается), а коробка перестаёт смотреть внутрь потока.

**Что делает z2k сам.** Имя подбирается не для каждого сайта, а для **сети**
(автономной системы), в которой он живёт: у одного провайдера пробивает одно имя
на всю сеть Hetzner, другое — на DigitalOcean, а для некоторых сетей не работает
ни одно. Раз в сутки в 03:30 (и при первом запуске службы) z2k проверяет линию
по списку опорных адресов, находит сети с обрывом и подбирает каждой имя.
Результат — в `state/tcp16_sni.txt`, карта сетей — в `lists/tcp16_nets.txt`.
...
Проверка занимает две-три минуты и идёт в фоне; трафик пользователя она не
трогает. Сетям, где обрыва нет, никакое имя не подставляется — правило
применяется только там, где блокировка действительно найдена.
...
│   │   └── z2k-alert.lua           # TLS-alert детектор + подстановка имени по сети (обрыв на 16 КБ)
