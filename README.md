...
На **новых прошивках Keenetic** «Протокол IPv6» и модули ядра Netfilter уже установлены по умолчанию — отдельно ставить ничего не нужно, и такого пункта в веб-интерфейсе может вообще не быть.

Только на **старых прошивках/роутерах** (если z2k не запускается после установки) в веб-интерфейсе Keenetic нужно доустановить компоненты:
...
После установки панель доступна в локальной сети на порту **8088** без авторизации. Она слушает на всех домашних сегментах роутера (основная сеть и гостевые), поэтому открывается по локальному адресу роутера в той сети, к которой вы подключены — например `http://192.168.1.1:8088/`. Точные адреса показывает пункт меню `[P]` и баннер установщика. Открывайте из локальной сети, а не из гостевого сегмента — гостевая сеть Keenetic по умолчанию изолирует доступ к сервисам роутера и порт 8088 на ней будет закрыт.
