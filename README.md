...
## Проверка домена (z2k-detect)

Меню `[Y]` и диагностика в веб-панели проверяют выбранный домен:
DNS → TCP → TLS → HTTP. Результат показывает, на какой стадии произошёл
отказ и есть ли домен в действующих списках. Проверка ничего в них не записывает.

Для запуска из консоли: `/opt/sbin/z2k-detect probe example.com`.
Также доступны отдельные проверки `classify`, `quic`, `voice`, `tcp16` и `dnsms`.

Экспериментальная фоновая автодетекция удалена. При обновлении или переустановке
её демон останавливается, автозапуск и накопленный `discovered-domains.txt`
удаляются. Пользовательские списки сохраняются. Отдельный автохостлист nfqws2
в меню `[L]` остаётся выключенным по умолчанию.

---

## Обрыв на 16 КБ (подбор имени по сетям)

Отдельный вид блокировки, который не лечится перебором стратегий: сайт
открывается, соединение живое, но страница **обрывается на середине** — приходит
первые 15–16 КБ и дальше тишина до таймаута. Перебор плеч тут бесполезен,
потому что рукопожатие проходит успешно: коробка провайдера пропускает начало и
режет поток дальше.

Пробивается тем, что вместе с настоящим приветствием отправляется поддельное — с
именем сайта, который у провайдера в белом списке. Поддельное — это копия
настоящего приветствия с заменённым именем (см. «Фейковый ClientHello — клон
настоящего» ниже). Настоящее имя остаётся настоящим (шифрование не ломается), а
коробка перестаёт смотреть внутрь потока.

**Это отдельная система, не ротация.** Автоподбор стратегий про этот блок ничего
не знает и на него не реагирует. Есть ли блок на линии и какое имя берёт какую
сеть, выясняет **проба линии** (`z2k-tcp16-probe.sh`): она ходит на опорные
адреса в разных сетях, ищет обрыв и подбирает каждой сети с обрывом своё имя.
Имя подбирается не для сайта, а для **сети** (автономной системы): у одного
провайдера пробивает одно имя на всю сеть Hetzner, другое — на DigitalOcean, а
для некоторых сетей не работает ни одно. Результат — `state/tcp16_sni.txt`,
карта сетей — `lists/tcp16_nets.txt`; в рантайме `lua/z2k-tcp16.lua` только
подставляет имя по этой карте.

**Когда проверяется.** Раз в сутки в 03:30, при первом запуске службы — и
вручную: на **дашборде** панели карточка «Обрыв на 16 КБ» показывает вердикт
последней пробы (с давностью), число сетей с обрывом и подобранных имён и
доехал ли обход до конфига; кнопка **«Пробить 16 КБ»** запускает пробу сейчас
и показывает её журнал. Проверка занимает две-три минуты и трафик пользователя
не трогает. Сетям, где обрыва нет, никакое имя не подставляется.

Замер на линии автора: сеть Hetzner отдавала 15 994 байта и вставала, с
подобранным именем — 163 654 байта за 0.64 с, полная страница.
