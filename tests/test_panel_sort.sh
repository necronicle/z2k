...
# ВТОРАЯ СТОРОНА ИНВЕРСИИ. Проверка выше смотрит только на шторку: зовёт ли она
# переданное действие. Но у косвенности две половины, и вторая — что кто-то
# это действие ПЕРЕДАЛ. Без неё замена openSortSheet(resortState) на
# openSortSheet() оставляет весь сьют зелёным: шторка исправно зовёт пустышку,
# выбор сохраняется, а таблица не пересортировывается. На телефоне это
# единственный способ сортировать — заголовки колонок там скрыты.
#
# Проверено ревью 08-15 исполнением: сломанное соединение давало 23 из 23 PASS.
# Парная проверка для второй инверсии (крючок входа) в test_panel_auth.sh есть
# с самого начала — здесь её просто забыли.
_pass=$(grep -n 'openSortSheet(' "$JS" | grep -v 'function openSortSheet' || true)
case "$_pass" in
    *'openSortSheet('[a-z]*) ok "страница передаёт шторке действие (иначе она зовёт пустышку)" ;;
    *) no "передача действия в шторку" "openSortSheet(действие)" \
          "$_pass — вызов без аргумента, сортировка на телефоне молча перестанет работать" ;;
esac
case "$hdr" in
