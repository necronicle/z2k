...
# --- Незарегистрированное устройство названо своей причиной -------------------
# Поле 2026-08-28: у человека device.json отсутствовал, движок падал на старте
# сутки подряд, а сводка писала «движок не запущен — selfheal поднимет его в
# течение минуты». Обещание было неправдой: без ключа не поднимет никогда.
if grep -q 'устройство не зарегистрировано у Cloudflare' "$DIAG"; then
    ok "нет ключа устройства — своя строка в сводке"
else
    no "нет ключа устройства — своя строка в сводке" "есть" "нет"
fi
# Порядок ветвей: без ключа status.json тоже отсутствует, поэтому проверка
# ключа обязана стоять РАНЬШЕ — иначе до неё не доходит никогда.
_dev_ln=$(grep -n 'device.json' "$DIAG" | grep -c .)
_ord=$(awk '/! -s \/opt\/etc\/z2k-warp\/device.json/ { d = NR }
            /нет status.json/                            { s = NR }
            END { print (d > 0 && d < s) ? "ok" : "bad" }' "$DIAG")
assert_eq "проверка ключа стоит раньше проверки status.json" "ok" "$_ord"
