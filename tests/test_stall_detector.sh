#!/bin/sh
# tests/test_stall_detector.sh — обёртка над lua-харнесом
# tests/test_stall_detector.lua, чтобы общий прогон (tests/run_all.sh) и CI
...
if [ -z "$LUA" ]; then
    printf "[PASS] stall_detector: пропущено (lua локально не установлена)\n"
    exit 0
fi

cd "$PROJECT_ROOT"
exec "$LUA" tests/test_stall_detector.lua
