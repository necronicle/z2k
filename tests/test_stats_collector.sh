...
# дедупликации не нужен»), и осмысленность сводки. До схлопывания он держался
# только на добросовестности клиента.
if [ "$(get OK_DUP)" = "204" ] && [ "$(get DUP_ROWS)" = "1" ]; then
    ok "400 одинаковых пар в одной выгрузке схлопнуты в одну строку"
else
    no "схлопывание дублей" "204 и 1 строка" \
       "$(get OK_DUP) и $(get DUP_ROWS) строк — один запрос выдаёт себя за 400 сэмплов"
fi
