...
# поэтому карта пересобирается каждую ночь, а не только при установке.
grep -q '03:30)' "$SCHED" \
    && ok "ночной слот заведён" \
    || bad "нет ночного слота — протухшие имена никто не заметит"

grep -q 'sni-refresh' "$SCHED" \
    && ok "ночью пересобирается карта имён" \
    || bad "ночной слот не запускает пробу"
...
grep -q 'z2k-sni-refresh.ts' "$SCHED" \
    ...
    || bad "нет дросселя — пачка перезапусков даст пачку проб"

# Половина карты хуже целой прежней: подменяем разом.
grep -q 'SNIOUT.new' "$PROBE" \
    && ok "карта имён подменяется разом, а не по частям" \
    || bad "карта пишется на месте — оборванный подбор оставит половину"

printf '\n--- имя ставится только сетям с блоком ---\n'
NETS="$DIR/files/lists/tcp16_nets.txt"
PROBE="$DIR/files/z2k-tcp16-probe.sh"

grep -q 'z2k_sni_for(desync' "$LUA" \
    && ok "имя выбирается по сети назначения" \
    || bad "имя ставится без учёта сети — уйдёт всему пулу одно и то же"

grep -q 'per-asn' "$PROBE" \
    && ok "проба подбирает имя на КАЖДУЮ сеть" \
    || bad "подбор не поимённый — одно имя на всех не работает"
