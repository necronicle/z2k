...
# --- 5. Миграция снимает пины 4pda, которые прошил r-77.2 --------------------
# Домен убран из HOSTS 19.08.2026 (блок по адресу продержался несколько часов),
# но людям он уже прошился релизом. Обновлять эти записи больше некому: адреса
# Cloudflare ротирует, и через недели они станут мёртвыми — сайт перестанет
# открываться у тех, у кого без нас открывался бы. Снимать их обязана миграция.
_mig=$(awk '/^cleanup_legacy_ip_hosts\(\)/,/^}/' "$INSTALL")
case "$_mig" in
    *4pda*) ok "миграция снимает прошитые ранее записи 4pda" ;;
    *)      bad "миграция не трогает 4pda — записи от r-77.2 останутся навсегда" ;;
esac

# Снимать надо при ЛЮБОМ адресе: их переписывало ежедневное обновление, и
# сверка с конкретным IP пропустила бы всё, кроме исходного.
case "$_mig" in
    *'"4pda.to"'*'\.4pda\.to$'*) ok "миграция ловит и апекс, и поддомены, независимо от адреса" ;;
    *) bad "миграция сверяет 4pda не по имени — часть записей уцелеет" ;;
esac

# И самого домена в рабочих списках больше быть не должно, иначе он вернётся
# следующим же прогоном обновления.
case " $hosts " in
    *" 4pda.to "*) bad "4pda всё ещё в HOSTS — обновление пропишет его заново" ;;
    *)             ok "4pda убран из HOSTS" ;;
esac

# --- 6. Rutracker-пины на месте ----------------------------------------------
