...
for f in "$CLEANUP" "$REFRESH" "$INSTALL" "$UTILS"; do
...
# Функция переехала в lib/utils.sh 31.08.2026: её зовёт шаг обновления
# cleanup-ip-hosts, а апдейтер подключает utils.sh и НЕ подключает install.sh —
# пока она лежала там, шаг у всех молча писал «функция недоступна». Ищем её там,
# где она теперь живёт, а не там, где привыкли.
_mig=$(awk '/^cleanup_legacy_ip_hosts\(\)/,/^}/' "$UTILS")
case "$_mig" in
