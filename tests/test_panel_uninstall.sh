...
if ! command -v node >/dev/null 2>&1; then
    ...
    # И финальная строка не должна выглядеть как сбой.
    if grep -q 'эту вкладку можно закрыть' "$APPJS"; then
        ok "финал удаления объявлен нормальным концом, а не обрывом"
    else
        no "финальная строка удаления" "«эту вкладку можно закрыть»" "нет"
    fi
fi
