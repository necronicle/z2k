#!/bin/sh
# tests/test_silent_drop_detector.sh — wrapper that runs the lua mock-harness
# tests/test_silent_drop_detector.lua so the shared runner (tests/run_all.sh)
...
if [ -z "$LUA" ]; then
    printf "[PASS] silent_drop_detector: skipped (lua not installed locally)\n"
    exit 0
fi
...
cd "$PROJECT_ROOT"
exec "$LUA" tests/test_silent_drop_detector.lua
