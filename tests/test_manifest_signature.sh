...
#
# НО СПРАШИВАТЬ ЭТО МОЖНО НЕ У ВСЯКОГО КОММИТА. Подпись накладывается один раз,
# в release.sh, и покрывает манифест целиком — включая карту сумм. Любая правка
# доставляемого файла карту меняет, а подписать её заново нечем: ключ лежит
# только у владельца, и CI к нему доступа не имеет по построению. Значит между
# релизами подпись СТАРЕЕТ, и это нормальное состояние рабочей ветки.
#
# Требовать сходимости на каждом коммите — значит держать CI красным всё время
# между релизами. Красный по умолчанию не читается вообще: именно так пропускают
# настоящие поломки. Поэтому спрашиваем только у релизного коммита — того, что
# двигает "current" относительно опубликованного.
_published_cur=$(git -C "$ROOT" show origin/z2k-enhanced:UPDATES.json 2>/dev/null \
    | sed -n 's/.*"current"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -1)
_tree_cur=$(sed -n 's/.*"current"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$ROOT/UPDATES.json" | head -1)

if [ -n "$_published_cur" ] && [ "$_published_cur" = "$_tree_cur" ]; then
    SKIP=$((SKIP+1))
    printf '[SKIP] подпись манифеста в дереве (рабочий коммит: current=%s не сдвинут — подписывает release.sh)\n' \
        "$_tree_cur"
elif [ -s "$ROOT/files/etc/z2k-update-pub.pem" ] && [ -s "$ROOT/UPDATES.json.sig" ]; then
