...
# --- 4. Выключенный руками туннель — не неисправность ------------------------
#
# Обратная сторона проверки №1. Выключив телеграм, человек штатно остаётся без
# правил редиректа и без процесса туннеля — и получал за это две строки:
# в сводке «телеграм не подключится», в деталях «expected 1». Собственное
# осознанное действие подавалось ему как поломка.
#
# Все проверки телеграма обязаны молчать разом: раньше защита стояла только на
# «процесса нет», а проверка правил висела ниже, вне этой ветки.
_guard=$(awk '/-x \/opt\/sbin\/tg-mtproxy-client/{print}' "$DIAG")
if printf '%s' "$_guard" | grep -q 'tg_user_disabled'; then
    ok "все проверки телеграма под одной защитой от ручного выключения"
else
    no "одна защита на все проверки" "tg_user_disabled в условии блока" "$_guard"
fi

_orphan=$(awk '/tg_user_disabled; then$/{f=1} f&&/^        fi$/{f=0} f&&/_add "правила редиректа/{print "внутри"}' "$DIAG")
if grep -q 'туннель выключен вручную — правил и не должно быть' "$DIAG"; then
    ok "детали не пишут «expected 1» при выключенном туннеле"
else
    no "ожидание в деталях зависит от состояния" "текст про ручное выключение" "нет"
fi
