...
const SCENARIOS = {
  ...
  outage: {
    ...
    async run() {
      ...
      check("сказано, что результат неизвестен", TOASTS.some(t => /неизвестно/.test(t)), TOASTS.join(" | "));
  ...
  job_refused: {
    ...
    async run() {
      ...
      check("панели не приписали пропажу связи",
            !TOASTS.some(t => /Связь с панелью пропала/.test(t)), TOASTS.join(" | "));
      check("сказано, что панель ответила ошибкой",
            TOASTS.some(t => /ответила ошибкой/.test(t)), TOASTS.join(" | "));
    },
  },
