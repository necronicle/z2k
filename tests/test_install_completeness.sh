...
for f in $LUA_USED; do
    ...
    LUA_DEFINED="$LUA_DEFINED $(grep -oE '^[a-z_][a-zA-Z_0-9]*[[:space:]]*=[[:space:]]*tls_mod\(' "$p" \
        | sed 's/[[:space:]]*=.*//' | tr '\n' ' ')"
    # Блоб, приготовленный в РАНТАЙМЕ и положенный в поле desync. Приём
    # апстримный: селектор кладёт готовый ClientHello в desync[<имя>], а штатный
    # `fake:blob=<имя>:optional` берёт его оттуда. Так сделан подбор имени из
    # белого списка — имя выбирается на ходу, поэтому глобалью объявить его
    # нельзя, и без этой ветки тест считал бы поле несуществующим блобом.
    LUA_DEFINED="$LUA_DEFINED $(grep -oE 'desync\[[^]]*"[a-z_][a-zA-Z_0-9]*"\][[:space:]]*=' "$p" \
