...
# --- 4. Класса сообщений про недоступную панель не существует ------------------
#
# Это и есть предмет спора: человеку в принципе не сообщается, что панель не
# отвечает. Задача идёт на роутере и от браузера не зависит.
_ghosts=0
for _s in 'панель ответила ошибкой' 'Связь с панелью пропала' 'панель временно недоступна' \
...
if [ "$_ghosts" = "0" ]; then
    ok "сообщений про недоступную панель в коде нет (кроме комментариев)"
else
    FAIL=$((FAIL + _ghosts))
fi
