...
#
# Политика «jsdelivr это фоллбэк, его отставание только предупреждает» была
# верна ДО подписи: отстающее зеркало отдавало файлы постарее, и это
# переживалось. С подписью и храповиком манифест и подпись — две независимые
# записи кэша, и стоит им разъехаться, попавший на это зеркало роутер получает
# не «постарее», а ЖЁСТКИЙ ОТКАЗ обновляться.
#
# Так и вышло с r-75.5 (2026-08-10): ран зелёный, purge «успешен», а jsdelivr
# ещё долго отдавал манифест r-75.4 со свежей подписью — и предупреждение об
# этом утонуло в зелёном ране, пока люди не могли обновиться.
_jsd_block=$(awk '/JSD="https:\/\/cdn\.jsdelivr\.net/,/^      - name:/' "$CDN")
if printf '%s' "$_jsd_block" | grep -q '::error::' \
   && printf '%s' "$_jsd_block" | grep -q 'exit 1' \
   && ! printf '%s' "$_jsd_block" | grep -q '::warning::.*подпись\|::warning::.*разных моментов'; then
    ok "несошедшаяся пара манифест+подпись на jsdelivr валит ран, а не пишет предупреждение"
else
    no "рассогласование корня доверия на jsdelivr — красное" \
       "::error:: + exit 1 без warning-ветки" "осталось предупреждение"
fi
# Успех purge-запроса не равен выселению — гейт обязан ДОЖДАТЬСЯ зеркала.
if printf '%s' "$_jsd_block" | grep -q 'sleep'; then
    ok "гейт ждёт, пока jsdelivr реально отдаст сошедшуюся пару (purge=успех ≠ выселено)"
else
    no "проверка jsdelivr повторяется с ожиданием" "цикл с sleep" "одна попытка"
fi
