...
# CD-гейт (HEAD == origin/z2k-enhanced + зелёный CI) в клоне не выполним и к
# делу не относится: здесь проверяется, что НЕотслеживаемые файлы не считаются
# грязным деревом. Гейт обходим ровно для этой проверки, штатным флагом.
out=$(Z2K_RELEASE_SKIP_CI_GATE=1 sh scripts/release.sh p-9999.1 patch "тест: только untracked" 2>&1); rc=$?
