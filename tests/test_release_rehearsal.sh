...
mk_fixture "$SB/base2" "$SB/cand2"
out2=$(sh "$ENGINE" --base "$SB/base2/zd" --candidate "$SB/cand2" \
       --from p-1 --to p-2 --interrupt-after 1 2>&1)
rc2=$?
...
assert_eq "исходный файл цел"             "старое" "$(cat "$SB/base2/zd/lua/a.lua" 2>/dev/null | tr -d '\n')"
assert_eq "привезённое лишнее убрано"     ""       "$(cat "$SB/base2/zd/lua/c.lua" 2>/dev/null | tr -d '\n')"
