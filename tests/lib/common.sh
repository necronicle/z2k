...
case "$_mode" in
    direct_fail)
        # Прямой хоп падает по ФАЗЕ СОЕДИНЕНИЯ, VPS-хоп при этом живой.
        # Нужен с тех пор, как прямой путь стал первым: без него бюджет
        # VPS-хопа не наблюдаем вовсе — до него просто не доходит.
        printf '000 0.000000'; exit 28 ;;
    m304)
