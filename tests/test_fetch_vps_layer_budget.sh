...
run() {
    ...
    env -i PATH="$TMP/bin:$Z2K_TEST_PATH" HOME="$TMP" \
        ...
        "$Z2K_TEST_SH" -c '
            ...
            # ходил к нему через один наш адрес, а лимиты там считаются по
...
for f in $COPIES; do
    ...
    # 3 -> 8 секунд, 26.08.2026. Три секунды — это ОДИН ретрансмит SYN: на
    # плохом канале рукопожатие в них не укладывается, и клиент уходил на
    # зеркала, хотя VPS был жив. Восемь покрывают два ретрансмита.
    #
    # Плату за мёртвый VPS держит размыкатель ниже по коду: без него подъём
    # бюджета умножился бы на полторы сотни файлов обновления.
    if [ "$b" = "8" ]; then
        ok "$f: бюджет коннекта Layer 0 = 8 с (было 3, до того 10)"
    else
        no "$f: бюджет коннекта Layer 0" "8" "${b:-нет --connect-timeout}"
    fi
...
b=$(vps_budget)
if [ "$b" = "8" ]; then
    ok "при мусоре в бюджете в curl уходит умолчание (8 с), а не мусор"
else
    no "мусор не доезжает до curl" "8" "${b:-нет}"
fi
...
# ── Размыкатель слоя 0 ───────────────────────────────────────────────────────
#
# ЗАЧЕМ. Бюджет коннекта поднят с 3 до 8 секунд: три — это один ретрансмит SYN,
# на плохом канале рукопожатие в них не укладывается, и клиент уходил на
# зеркала при ЖИВОМ VPS. Но восемь секунд в две попытки на КАЖДЫЙ файл — это
# сорок минут ожидания на обновлении из полутора сотен файлов, когда VPS мёртв.
#
# Поэтому подряд идущие отказы ФАЗЫ СОЕДИНЕНИЯ считаются, и после второго слой
# отключается до конца прогона. Проверяем именно это: второй вызов в том же
# процессе уже не должен трогать VPS.
run2() {
    _file="$1"; _mode="$2"
    ...
    env -i PATH="$TMP/bin:$Z2K_TEST_PATH" HOME="$TMP" \
        ...
        "$Z2K_TEST_SH" -c '
            ...
            _z2k_curl_doh() { return 1; }
            _z2k_manifest_sha() { printf ""; }
            z2k_fetch "/один.txt" "$SB/d1" >/dev/null 2>&1
            a=$(grep -c -- "--resolve" "$CURL_LOG" 2>/dev/null); [ -n "$a" ] || a=0
            : > "$CURL_LOG"
            z2k_fetch "/два.txt" "$SB/d2" >/dev/null 2>&1
            b=$(grep -c -- "--resolve" "$CURL_LOG" 2>/dev/null); [ -n "$b" ] || b=0
            printf "первый=%s второй=%s" "$a" "$b"
        ' 2>/dev/null
}
...
for f in z2k.sh lib/utils.sh files/z2k-update-lists.sh; do
    ...
    case "$r" in
        "первый=2 второй=0")
            ok "$f: после двух отказов подряд слой 0 больше не пробуется" ;;
        NOFN) no "$f: z2k_fetch извлечена" "определение есть" "не найдено" ;;
        *) no "$f: размыкатель слоя 0" "первый=2 второй=0" "$r" ;;
    esac
done

# Обратная сторона: успех обязан обнулять счёт, иначе редкие одиночные сбои
# накопятся за прогон и выключат слой на ровном месте. vps_flap валит ТОЛЬКО
# первый вызов — значит второй запрос обязан снова пойти в VPS.
for f in z2k.sh lib/utils.sh files/z2k-update-lists.sh; do
    r=$(run2 "$f" vps_flap)
    case "$r" in
        "первый=2 второй=1")
            ok "$f: успех обнуляет счёт, слой 0 остаётся в строю" ;;
        NOFN) no "$f: z2k_fetch извлечена" "определение есть" "не найдено" ;;
        *) no "$f: успех обнуляет счёт размыкателя" "первый=2 второй=1" "$r" ;;
    esac
done
