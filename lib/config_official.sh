...
AUSTERUS_OPT
    ...
    # `allow_nohost=1` — наш wrapper-аргумент (см. z2k-autocircular.lua):
    # Discord UDP handshake без SNI → подмена track.hostname на "nohost"
    # → shared rotation state `discord_udp\tnohost` в state.tsv.
    discord_udp="--filter-udp=50000-50099,1400,3478-3481,5349,19294-19344 --filter-l7=discord,stun --in-range=-d100 --out-range=-d100 --payload=quic_initial,discord_ip_discovery --lua-desync=circular:fails=3:time=60:udp_in=1:udp_out=4:key=discord_udp:nld=2:allow_nohost=1 --lua-desync=fake:payload=all:blob=quic_dbankcloud:repeats=6:strategy=1 --lua-desync=fake:payload=all:blob=quic_dbankcloud:repeats=4:strategy=2 --lua-desync=fake:payload=all:blob=quic_dbankcloud:repeats=8:strategy=3 --lua-desync=fake:payload=all:blob=quic_dbankcloud:repeats=6:ip_autottl=-2,3-20:strategy=4 --lua-desync=fake:payload=all:blob=fake_default_quic:repeats=6:strategy=5 --lua-desync=fake:payload=all:blob=quic5:repeats=6:strategy=6"
