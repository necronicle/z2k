...
generate_nfqws2_opt_from_strategies() {
    ...
    # --------------------------------------------------------------------------
    # ПРОБОЙ БЕЛОГО СПИСКА ЧУЖИМ ИМЕНЕМ (профиль sni_wl) — ПЕРЕД rkn_tcp
    # --------------------------------------------------------------------------
    #
    # Класс блокировки, который наш ротатор не лечит в принципе. Замер на линии
    # владельца, hetzner.com (AS24940): контрольное плечо отдаёт 200 и ровно
    # 15994 байта, дальше поток стоит до таймаута. То же плечо с фейковым
    # ClientHello, несущим чужое имя (disk.rzd.ru), — 163654 байта за 0.64 с,
    # полная страница. Тот же результат с akashi.vk-portal.net, и одинаковый с
    # hostfakesplit и с multisplit:seqovl=1: несущая часть — ИМЯ, а не разрез.
    # Белый список коробки узкий и конкретный: прямой пробой на тот же адрес
    # disk.rzd.ru и akashi.vk-portal.net отвечают за 0.2 с, а www.hetzner.com и
    # ya.ru висят в таймауте — «российское имя» само по себе не пропуск. Поэтому
    # имя не зашито в код: его выбирает проба по lists/sni_wl_candidates.txt и
    # кладёт победителя в отдельный файл.
    #
    # ПОЧЕМУ ФЕЙК БЕЗ :strategy= И ПОЧЕМУ ОН СТОИТ ДО circular. Шестнадцати-
    # килобайтный обрыв не видит ни один наш детектор: 400 обращений детекторов
    # за три зависших захода дали ноль событий, то есть ротация на этом классе
    # не запустится никогда (да и в норме она редка — 59 из 62 хостов в
    # state.tsv сидят на плече 1). Значит имя обязано подставляться на ЛЮБОМ
    # плече, а не быть одним из плеч. Инстанс без аргумента strategy= circular
    # молча выбрасывает (docs/manual.md, «Оркестраторы → circular»:
    # «Инстансы, не имеющие аргумента strategy, circular не вызывает»;
    # lua/zapret-auto.lua исполняет в цикле plan_instance_pop только те, у кого
    # strategy совпала с hrec.nstrategy), а весь хвост плана circular забирает
    # себе. Единственное место, где безымянный инстанс выполняется всегда, —
    # ДО circular, там работает линейный оркестратор. Фейк, поставленный после
    # circular, не выстрелит ни разу; разрез при этом продолжает ротироваться
    # штатно, потому что весь арсенал приезжает импортом уже за circular.
    #
    # ПОЧЕМУ ПЕРЕСЕЧЕНИЕ, А НЕ ОБЪЕДИНЕНИЕ. Профиль совпадает, только когда
    # выполнено всё сразу: адрес в поражённой сети (--ipset), домен уже в
    # списках блокировок (те же хостлисты РКН, что у rkn_tcp), домен не в белом
    # списке пользователя. Коробка привязана к СЕТИ назначения — на кампании из
    # 507 замеров по 412 сетям согласие вердиктов внутри /16 составило 81.3%, —
    # это и задаёт гранулярность файла сетей. Шире брать нечего: фейк несёт
    # ЧУЖОЕ имя, и вешать его на весь TLS-трафик значит подставлять это имя в
    # потоки, к блокировке отношения не имеющие. Отдельный key=sni_wl держит
    # своё состояние ротации и не размывает выбор плеча основного пула.
    #
    # Профиль стоит ПЕРЕД rkn_tcp намеренно: движок выбирает ровно один профиль
    # по первому совпадению фильтров, а условия sni_wl — надмножество условий
    # rkn_tcp, так что после него он не сработал бы ни разу.
    #
    # Профиль НЕ ГЕНЕРИРУЕТСЯ ВООБЩЕ, пока пуст любой из двух файлов — у тех,
    # кто кампанию не гонял, конфиг не меняется ни на байт. Гейт считает
    # СОДЕРЖАТЕЛЬНЫЕ строки, а не размер файла: `[ -s ]` (как у cf_extra ниже)
    # пропускает файл из одних комментариев, и профиль уехал бы в конфиг с
    # ipset'ом, не совпадающим ни с чем.
    #
    # ЧАСОВОЙ 192.0.2.0/24 СЕТЬЮ ДЛЯ ГЕЙТА НЕ СЧИТАЕТСЯ. Подборщик держит эту
    # строку (RFC 5737 TEST-NET-1) в файле сетей ВСЕГДА, потому что файлу
    # нельзя опускаться до пустого: пустой ipset движок трактует как ОТСУТСТВИЕ
    # фильтра, и профиль вместо поражённых сетей начинает ловить весь TLS
    # подряд — воспроизведено на живом роутере. Но раз строка есть всегда,
    # засчитай её содержательной — и профиль включится у каждого, у кого не
    # найдено НИ ОДНОЙ поражённой сети, то есть исчезнет ровно та защита 99%,
    # ради которой гейт и написан. Отсюда: считаем настоящие сети, часовой из
    # подсчёта вычтен. Из самого файла он при этом не вырезается — в --ipset
    # файл уезжает целиком, и часовой там безвреден: реального трафика в
    # TEST-NET-1 не бывает по определению диапазона.
    local Z2K_SNI_WL_FAKE sni_wl_nets sni_wl_name_file sni_wl_net sni_wl_name
    local sni_wl_sentinel
    Z2K_SNI_WL_FAKE=$(safe_config_read "Z2K_SNI_WL_FAKE" "${ZAPRET2_DIR:-/opt/zapret2}/config" "1")
    sni_wl_nets="${lists_dir}/sni_wl_nets.txt"
    sni_wl_name_file="${lists_dir}/sni_wl_name.txt"
    sni_wl_sentinel="192.0.2.0/24"

    # Содержательные строки файла. Комментарии '#' и пустые строки допустимы в
    # обоих файлах — ровно так их допускает и сам --ipset.
    sni_wl_content_lines() {
        [ -s "$1" ] || return 1
        sed -e 's/#.*$//' -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//' "$1" 2>/dev/null \
            | grep -v '^$'
    }

    sni_wl_net=""
    sni_wl_name=""
    if [ "$Z2K_SNI_WL_FAKE" = "1" ]; then
        sni_wl_net=$(sni_wl_content_lines "$sni_wl_nets" \
            | grep -v -x -F -- "$sni_wl_sentinel" | head -1)
        sni_wl_name=$(sni_wl_content_lines "$sni_wl_name_file" | head -1)
    fi

    # Имя уезжает в командную строку демона как есть, а разделители там ':' и
    # ','. Пробел, кавычка или двоеточие в имени — это не «фейк без имени», это
    # не стартовавший nfqws2, то есть обход, лежащий целиком. Пропускаем только
    # то, что является именем хоста: буквы, цифры, точки, дефисы, без пустых
    # меток, без крайних '.'/'-' и не длиннее предела DNS.
    if [ -n "$sni_wl_name" ]; then
        local _sni_bad=""
        case "$sni_wl_name" in
            *[!A-Za-z0-9.-]*|.*|-*|*.|*-|*..*) _sni_bad="1" ;;
        esac
        [ "${#sni_wl_name}" -gt 253 ] && _sni_bad="1"
        if [ -n "$_sni_bad" ]; then
            echo "WARN: sni_wl: not a hostname in $sni_wl_name_file (skip profile)" 1>&2
            sni_wl_name=""
        fi
    fi

    if [ -n "$sni_wl_net" ] && [ -n "$sni_wl_name" ]; then
        local sni_fake sni_circ sni_strats
        sni_fake="--lua-desync=fake:payload=tls_client_hello:dir=out:blob=fake_default_tls:repeats=8:tcp_ts=-1000:tls_mod=rnd,dupsid,sni=${sni_wl_name}"
        # Паритет с арсеналом: все его fake-инстансы уже прошли через
        # inject_z2k_dynamic_ttl выше по функции. Наш собран позже — гоним его
        # через ту же функцию и тот же опт-аут, иначе единственный фейк без
        # подобранного TTL торчал бы из общего строя.
        [ "$Z2K_DYNAMIC_TTL" != "0" ] && sni_fake=$(inject_z2k_dynamic_ttl "$sni_fake")
        if [ -n "$rkn_circ" ]; then
            # Копируется только circular-токен: в нём и есть отличие (key=),
            # арсенал приезжает импортом. $rkn_head — это всё до circular, то
            # есть фильтры и состояние разборщика; фейк встаёт сразу за ним и
            # потому оказывается первым инстансом профиля.
            sni_circ=$(printf '%s' "$rkn_circ" | sed 's/key=rkn_tcp/key=sni_wl/')
            add_hostlist_line "${extra_strats_dir}/TCP/RKN/List.txt" \
                "--ipset=${sni_wl_nets} $wl_excl $rkn_hostlists $rkn_head $sni_fake $sni_circ --import=$rkn_tpl --new"
        else
            # Плоская форма (Z2K_NFQWS2_TEMPLATES=0 или пул без circular).
            # Фейк вставляется перед ПЕРВЫМ инстансом разбором токенов, а не
            # sed'ом по строке: имя пришло из файла и в шаблон замены попадать
            # не должно. Перед первым, а не перед circular, потому что пул
            # может быть и вовсе без оркестратора — тогда инстансы исполняются
            # по порядку и фейк всё равно обязан быть первым, но уже после
            # опций головы, иначе --payload/--out-range на него не подействуют.
            local _t _seen=""
            sni_strats=""
            set -f
            for _t in $(printf '%s' "$rkn_tcp" | sed 's/key=rkn_tcp/key=sni_wl/'); do
                if [ -z "$_seen" ]; then
                    case "$_t" in
                        --lua-desync=*)
                            _seen="1"
                            sni_strats="${sni_strats:+$sni_strats }$sni_fake" ;;
                    esac
                fi
                sni_strats="${sni_strats:+$sni_strats }$_t"
            done
            set +f
            [ -n "$_seen" ] || sni_strats="${sni_strats:+$sni_strats }$sni_fake"
            add_hostlist_line "${extra_strats_dir}/TCP/RKN/List.txt" \
                "--ipset=${sni_wl_nets} $wl_excl $rkn_hostlists $sni_strats --new"
        fi
    fi
