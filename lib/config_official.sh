...
generate_nfqws2_opt_from_strategies() {
    ...
    add_hostlist_line "${extra_strats_dir}/TCP/RKN/List.txt" "--filter-tcp=80 --hostlist-exclude=${lists_dir}/whitelist.txt --hostlist=${extra_strats_dir}/TCP/RKN/List.txt --in-range=-s5556 --lua-desync=circular:fails=3:retrans=2:maxseq=16384:time=60:reset:key=http_rkn:nld=2 --in-range=x --payload=http_req --lua-desync=fake:blob=0x0E0E0F0E:ip_autottl=2:tcp_md5:strategy=1 --lua-desync=fake:blob=0x0E0E0F0E:tcp_md5:strategy=2 --lua-desync=multidisorder:pos=host+1:strategy=2 --lua-desync=fake:blob=0x0E0E0F0E:tcp_md5:strategy=3 --lua-desync=multisplit:pos=host+1:seqovl=2:strategy=3 --lua-desync=fake:blob=0x0E0E0F0E:ip_autottl=2:tcp_md5:strategy=4 --lua-desync=fakedsplit:pos=host+1:strategy=4 --lua-desync=fake:blob=0x0E0E0F0E:ip_autottl=2:badsum:strategy=5 --lua-desync=fakedsplit:pos=host+1:strategy=5 --lua-desync=fakedsplit:pos=host+1:ip_ttl=4:repeats=16:strategy=6 --new"
