...
generate_nfqws2_opt_from_strategies() {
    ...
    # Strategy 1 (MGTS):           fake only (ip_autottl=2, tcp_md5)
    # Strategy 2 (#699 recommend): fake + multidisorder (tcp_md5) — disorder approach
    # Strategy 3 (z4r):            fake + multisplit (host+1, seqovl=2, tcp_md5)
    # Strategy 4 (remittor):       fake + fakedsplit (ip_autottl=2, tcp_md5)
    # Strategy 5 (remittor v71):   fake + fakedsplit (ip_autottl=2, badsum)
    # Strategy 6 (Intersvyaz):     fakedsplit (ip_ttl=4, repeats=16)
    add_hostlist_line "${extra_strats_dir}/TCP/RKN/List.txt" "--filter-tcp=80 --hostlist-exclude=${lists_dir}/whitelist.txt --hostlist=${extra_strats_dir}/TCP/RKN/List.txt --in-range=-s5556 --lua-desync=circular:fails=3:retrans=2:maxseq=16384:time=60:reset:key=http_rkn:nld=2 --in-range=x --payload=http_req --lua-desync=fake:blob=0x0E0E0F0E:ip_autottl=2:tcp_md5:strategy=1 --payload=http_req --lua-desync=fake:blob=0x0E0E0F0E:tcp_md5:strategy=2 --lua-desync=multidisorder:pos=host+1:strategy=2 --payload=http_req --lua-desync=fake:blob=0x0E0E0F0E:tcp_md5:strategy=3 --lua-desync=multisplit:pos=host+1:seqovl=2:strategy=3 --payload=http_req --lua-desync=fake:blob=0x0E0E0F0E:ip_autottl=2:tcp_md5:strategy=4 --lua-desync=fakedsplit:pos=host+1:strategy=4 --payload=http_req --lua-desync=fake:blob=0x0E0E0F0E:ip_autottl=2:badsum:strategy=5 --lua-desync=fakedsplit:pos=host+1:strategy=5 --payload=http_req --lua-desync=fakedsplit:pos=host+1:ip_ttl=4:repeats=16:strategy=6 --new"
