...
AUSTERUS_OPT
    ...
    # Keep plain HTTP on the master-compatible path.
    #
    # The enhanced HTTP classifier can see only headers/early reply bytes
    # inside --in-range=-s5556. On mid-stream stalls (headers arrive, body
    # dies later) adding http_reply visibility can pin a strategy as working
    # before the broken body transfer is observable. Limit http_rkn to the
    # original request/control-packet view and let standard redirect/RST/
    # retrans logic drive rotation.
    #
    # Also intentionally keep HTTP scoped to the canonical RKN list only.
    # extra-domains.txt may contain TLS/CDN rescue entries whose plain HTTP
    # traffic is worse when forced through these 7 request-side strategies.
