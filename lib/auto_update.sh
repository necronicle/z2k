...
au_fetch_manifest() {
    ...
    [ -s "$out" ] || return 1
    au_verify_manifest_signature "$out" || return 1
    return 0
}

# Проверить подпись манифеста (issue #28).
#
# files_sha256 закрывает целостность КАЖДОГО файла, но сам манифест до сих пор
# принимался на веру: кто подменил UPDATES.json, тот подменил и карту хешей, и
# получил root на всём парке. Транспорту доверять нельзя ни в каком звене — среди
# зеркал есть gh-proxy, который терминирует TLS у себя.
#
# Подпись Ed25519 над всем файлом целиком. Приватный ключ в репозитории
# отсутствует и живёт только у мейнтейнера; публичный ставится на роутер при
# установке. Подменить манифест теперь мало — нужен ключ, которого в репозитории
# нет, поэтому компрометация GitHub этого больше не даёт.
#
# Ключ не может быть заменён враждебным обновлением: проверка стоит ДО того, как
# из манифеста будет применён хоть один файл.
Z2K_AU_PUBKEY="${Z2K_AU_PUBKEY:-/opt/zapret2/etc/z2k-update-pub.pem}"
au_verify_manifest_signature() {
    local manifest="$1"
    local sig="${manifest}.sig"

    if [ ! -s "$Z2K_AU_PUBKEY" ]; then
        # Роутер, установленный до появления подписи, ключа ещё не имеет. Ронять
        # ему обновления нельзя — он именно этим обновлением ключ и получит.
        au_log "manifest: публичный ключ не найден ($Z2K_AU_PUBKEY) — подпись не проверяется"
        return 0
    fi
    if ! command -v openssl >/dev/null 2>&1; then
        au_log "manifest: openssl отсутствует — подпись не проверяется"
        return 0
    fi

    rm -f "$sig"
    if command -v z2k_fetch >/dev/null 2>&1; then
        z2k_fetch "${Z2K_AU_MANIFEST_URL}.sig" "$sig" 2>/dev/null
    else
        curl -fsSL --max-time 30 "${Z2K_AU_MANIFEST_URL}.sig" -o "$sig" 2>/dev/null
    fi
    if [ ! -s "$sig" ]; then
        # Ключ есть, а подписи нет — это либо срыв подписи по пути, либо релиз
        # без неё. И то и другое означает непроверяемый манифест: отказываем.
        au_log "manifest: ОТКАЗ — публичный ключ есть, а подписи нет"
        rm -f "$sig"
        return 1
    fi

    if openssl pkeyutl -verify -pubin -inkey "$Z2K_AU_PUBKEY" \
         -rawin -in "$manifest" -sigfile "$sig" >/dev/null 2>&1; then
        au_log "manifest: подпись верна"
        rm -f "$sig"
        return 0
    fi

    # Провал проверки — ещё не доказательство подмены. В ту же ветку попадают
    # «этот openssl не умеет ed25519» (LibreSSL, OpenSSL до 3.0 — там нет -rawin)
    # и «ключ на диске побился при записи». Отвергать манифест в этих случаях
    # нельзя: переустановка, которая переписала бы ключ, идёт ПОСЛЕ этого гейта,
    # и роутер уходит в вечный отказ, лечится только руками по SSH.
    #
    # Различаем по способности загрузить ключ: если openssl не может прочитать
    # заведомо валидный публичный ключ — проблема на нашей стороне, не в
    # манифесте. Обвиняем в подмене, только когда проверка реально отработала.
    if ! openssl pkey -pubin -in "$Z2K_AU_PUBKEY" -noout >/dev/null 2>&1; then
        au_log "manifest: ключ не читается этим openssl (нет ed25519 или файл повреждён) — подпись не проверяется"
        rm -f "$sig"
        return 0
    fi

    au_log "manifest: ОТКАЗ — подпись не сошлась, манифест подменён или повреждён"
    rm -f "$sig"
    return 1
}
