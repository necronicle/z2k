...
au_apply_converge() {
    ...
    # Пин на неизменяемую ссылку целевой версии — как в au_apply_patch. Здесь его
    # не было, и au_repo_base брала Z2K_AU_TARGET_REF из ОКРУЖЕНИЯ: переустановка
    # экспортирует её и перезапускает планировщик и панель, а те передают её
    # каждому следующему прогону. Роутер, переустановленный ночью на p-84.18, с
    # 16.09 качал все файлы с тега p-84.18 при суммах от новой версии — «не
    # совпало» на всех четырёх источниках и откат, девять ночей подряд. Пустой
    # ответ (старый манифест) тоже присваиваем: он затирает унаследованное.
