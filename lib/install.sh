...
fw_nfqws_post4()
{
	...
	if [ -n "$wan" ]; then
		...
		[ -n "$Z2K_DEBUG" ] && echo "WARN: WAN iface not found (IPv4), applying without -o"
		_fw_nfqws_post4 "$1" "$2" "$3" ""
	fi
}
...
fw_nfqws_post6()
{
	...
	if [ -n "$wan" ]; then
		...
		[ -n "$Z2K_DEBUG" ] && echo "WARN: WAN iface not found (IPv6), applying without -o"
		_fw_nfqws_post6 "$1" "$2" "$3" ""
	fi
}
...
fw_nfqws_pre4()
{
	...
	if [ -n "$wan" ]; then
		...
		[ -n "$Z2K_DEBUG" ] && echo "WARN: WAN iface not found (IPv4), applying without -i"
		_fw_nfqws_pre4 "$1" "$2" "$3" ""
	fi
}
...
fw_nfqws_pre6()
{
	...
	if [ -n "$wan" ]; then
		...
		[ -n "$Z2K_DEBUG" ] && echo "WARN: WAN iface not found (IPv6), applying without -i"
		_fw_nfqws_pre6 "$1" "$2" "$3" ""
	fi
}
...
case "$1" in
    ...
    list_table)
        ...
        ;;
    *)
        echo "Usage: cat > "$INIT_SCRIPT" <<'INIT_EOF'
#!/bin/sh
# /opt/etc/init.d/S99zapret2
# Адаптация официального init.d/openwrt/zapret2 для Keenetic
# Использует модули common/ и config файл вместо hardcoded настроек
...
# Keenetic-specific: упрощенные версии fw_nfqws_post4/post6
# В отличие от OpenWrt, не требуют network.sh и определения WAN интерфейсов
# Работают через routing table (без явного указания -o interface)
...
fw_nfqws_post4()
{
	...
	# $2 - filter
	# $3 - queue number
	# Вызывает _fw_nfqws_post4 БЕЗ указания WAN интерфейсов (4-й параметр пустой)
	# iptables сам определит маршрутизацию
	_fw_nfqws_post4 "$1" "$2" "$3" ""
}
...
exit $?
INIT_EOF {start|stop|restart|status|start_fw|stop_fw|restart_fw|start_daemons|stop_daemons|restart_daemons|reload_ifsets|list_ifsets|list_table}"
        exit 1
        ;;
esac
