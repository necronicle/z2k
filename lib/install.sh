...
step_build_zapret2() {
    ...
    local fallback_url="https://github.com/necronicle/zapret2-z2k/releases/download/v0.9.4.7-z2k-r5/zapret2-v0.9.4.7-z2k-r5-openwrt-embedded.tar.gz"

    local openwrt_url
    if [ $? -ne 0 ]; then
        print_warning "API недоступен, использую fallback версию v0.9.4.7-z2k-r5..."
        ...
        if [ -z "$openwrt_url" ]; then
            print_warning "Не найден в API, использую fallback v0.9.4.7-z2k-r5..."
            openwrt_url="$fallback_url"
        fi
    fi
