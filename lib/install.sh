...
step_build_zapret2() {
    ...
    local openwrt_url
    if [ $? -ne 0 ]; then
        print_warning "API недоступен, использую fallback версию v0.9.4.2..."
        openwrt_url="https://github.com/bol-van/zapret2/releases/download/v0.9.4.2/zapret2-v0.9.4.2-openwrt-embedded.tar.gz"
    else
        # Парсим URL из JSON
        ...
        if [ -z "$openwrt_url" ]; then
            print_warning "Не найден в API, использую fallback v0.9.4.2..."
            openwrt_url="https://github.com/bol-van/zapret2/releases/download/v0.9.4.2/zapret2-v0.9.4.2-openwrt-embedded.tar.gz"
        fi
    fi
