...
download_openwrt_embedded_release() {
    ...
    if [ -n "$_vps_resolve" ]; then
        ...
        # слое (lib/utils.sh, Z2K_FETCH_VPS_CONNECT_TIMEOUT), причём в две
        # попытки. Разные бюджеты на один и тот же хоп были просто недосмотром.
        #
        # Замер 31.08.2026, 30 запросов на каждый бюджет: при трёх секундах
        # 29 успешных, при восьми — 30. Само соединение занимает 0.11 с в
        # среднем и 0.68 с в худшем, то есть срывается не установка связи, а
        # хвост. Цена ошибки несимметрична: лишние секунды ожидания против
        # потери первичного пути, поэтому берём больший бюджет.
        if curl -fsSL --connect-timeout "${Z2K_VPS_CONNECT_TIMEOUT:-8}" \
