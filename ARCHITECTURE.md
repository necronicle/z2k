...
**Всё в `lib/*` — это генераторы времени установки.** Они не «раскладываются
по местам», а производят конфиг и init-скрипты. Отсюда правило:

| Что изменилось | Тип релиза |
|---|---|
| `files/`, списки, lua, вебпанель | `patch` |
| **`lib/*`** | **`reinstall`** |
| **конфиги** (то, что `lib/*` генерирует) | **`reinstall`** |
| init-скрипты из `files/init.d/` | `patch` — см. ниже |
| любой `*/builds/*` | **`reinstall`** |

Про init-скрипты: патчем доставляются и перезапускаются только те, для которых
это заведено явно — `S98tg-tunnel`, `S97z2k-http-tunnel`, `S96z2k-rt-proxy`,
`S51z2k-warp`, `S99z2k-scheduler`, `S98z2k-detect`. У каждого есть и цель в
`au_install_paths` (иначе файл лёг бы в `${zd}/init.d/` и не доехал до системы —
так уже терялся r-59.9), и запись в `restart_set` в `au_apply_patch` (иначе новый
файл лежал бы на месте, а работал старый процесс). Init-скрипт вне этого списка —
`reinstall`.

Бинарники — всегда reinstall, потому что цель зависит от архитектуры роутера,
...
тулчейн пиннится на `go1.25.12` (на 1.26.x ловили `_ENOSYS`), а `z2k-detect`
