...
again:
	...
	var fns []ReceiveFunc
	if v4conn != nil {
		// z2k: UDP GSO/GRO выключены. GRO отдаёт склейку нескольких датаграмм
		// одним буфером, а буферы у нас MaxSegmentSize=2048 (device/
		// queueconstants_default.go) — склейка в него не влезет. GSO без GRO
		// смысла не имеет, а с буферами в 2 КБ ещё и аллоцировал бы на каждую
		// отправку.
