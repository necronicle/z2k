...
//
// WG :2408 → WG по каждому запасному порту из регистрации.
//
// MASQUE-h2 из автоматической лестницы снят 2026-08-24 по замеру: он
// поднимается, рапортует готовность и не возит ничего (см. New).
...
func New(ep account.Endpoint, start *account.Step) *Ladder {
	...
	// MASQUE В АВТОМАТИЧЕСКОЙ ЛЕСТНИЦЕ БОЛЬШЕ НЕТ, и это снятие по замеру.
	//
	// Замер 2026-08-24 на роутере владельца: туннель h2 поднимается, статус
	// говорит ready — и не возит НИЧЕГО. С маршрутом через интерфейс: ICMP
	// 100% потерь, TCP код 000, rx=895 при tx=2617. Тот же бинарник с VPS
	// (colo=FRA) возит всё, TCP отвечает за 10 мс, — значит реализация цела,
	// глушит его сеть.
	//
	// Пока шаг стоял в лестнице, он был не страховкой, а ловушкой: при
	// заблокированном WG движок падал на h2, объявлял готовность, маршрут
	// поднимался — и человек оставался без интернета на всём, что завернул в
	// WARP. Ровно эта жалоба и пришла из поля. Честный отказ лучше: тогда
	// срабатывает fail-open и трафик идёт напрямую.
	//
	// Шаг никуда не делся: `-force-transport h2` поднимает его вручную, и на
	// сети, где он работает, им можно пользоваться. Вернуть его в автоматику
	// имеет смысл только вместе с ротацией плеч для безхостового потока
	// (hostkey=z2k_nohost_key) — тогда появится перебор, который сегодня
	// невозможен: без хоста ротатор не заводит запись и стратегию не меняет.
	// Ручной запуск идёт мимо этой функции — ladder.NewFixed, — поэтому здесь
	// шаг просто не появляется, и сигнатуру менять не нужно.
