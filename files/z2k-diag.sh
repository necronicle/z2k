...
# Сколько стратегий РЕАЛЬНО доехало до движка.
#
# Читаем живую командную строку процесса, а не config: между ними стоит
# генератор (create_official_config → generate_nfqws2_opt_from_strategies), и
# расхождение здесь — это и есть искомая поломка. Конфиг может быть полон, а в
# движке пусто, и наоборот.
#
# Вынесено в функцию, потому что нужно в двух местах: в сводке «что не так» и в
# разделе service. Дублировать разбор /proc нельзя — разъедется.
#
# Печатает "<всего> <из них circular>"; возвращает 1, если процесса нет или
# /proc недоступен. Молчание здесь честнее догадки: не смогли прочитать —
# ничего не утверждаем.
nfqws_strategy_counts() {
    local _p _src _cmd _all _circ
    ...
    if [ -z "$_src" ]; then
        _p=$(pgrep -f 'nfq2/nfqws2' 2>/dev/null | head -1)
        [ -n "${_p:-}" ] || return 1
    ...
    _cmd=$(tr '\0' '\n' < "$_src" 2>/dev/null)
    [ -n "$_cmd" ] || return 1
    _all=$(printf '%s\n' "$_cmd" | grep -c -- '--lua-desync=')
    _circ=$(printf '%s\n' "$_cmd" | grep -c -- '--lua-desync=circular')
    case "${_all:-}" in ''|*[!0-9]*) _all=0 ;; esac
    case "${_circ:-}" in ''|*[!0-9]*) _circ=0 ;; esac
    printf '%s %s' "$_all" "$_circ"
}
...
print_service() {
    ...
    if [ -n "$nfqws_pids" ]; then
        ...
        local _sc
        if _sc=$(nfqws_strategy_counts); then
            printf 'стратегий в движке: %s (из них circular: %s)\n' \
                "${_sc%% *}" "${_sc##* }"
        else
            printf 'стратегий в движке: (не прочитать /proc)\n'
        fi
        ...
        for _pool in TCP/RKN TCP/YT TCP/YT_GV UDP/YT; do
            ...
            if [ -s "$_sf" ]; then
                _sn=$(grep -cvE '^[[:space:]]*(#|$)' "$_sf" 2>/dev/null)
                case "${_sn:-}" in ''|*[!0-9]*) _sn=0 ;; esac
                printf 'strategy %-9s: %s строк\n' "$_pool" "$_sn"
            elif [ -e "$_sf" ]; then
...
"; }
    ...
    # строке ни одной стратегии. Снаружи это выглядит как «z2k включён, а всё
    ...
    if pgrep -f 'nfq2/nfqws2' >/dev/null 2>&1; then
        local _scv
        if _scv=$(nfqws_strategy_counts); then
            case "$_scv" in
                '0 '*)
                    _add "движок работает, но НИ ОДНОЙ стратегии в нём нет — обход не применяется ни к чему; проверьте extra_strats/*/Strategy.txt и переустановите z2k" ;;
                *' 0')
