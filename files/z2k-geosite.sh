...
fetch_to_tmp() {
    ...
    local _vps_try=0 _vps_tries _vps_raw _vps_conn="" _ct _vps_rc
    ...
    while :; do
        ...
        #
        # %{time_connect} нужен целым: у соединения, которое УСТАНОВИЛОСЬ и
        # потом встало (обрыв по --speed-time), rc=28, а время коннекта
        # НЕнулевое. Затирать его нельзя — иначе гейт повтора ниже примет
