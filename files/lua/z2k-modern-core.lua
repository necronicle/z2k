...
-- ----------------------------------------------------------------------------
-- 16KB-block bypass primitives — ported from
-- github.com/ALFiX01/GoodbyeZapret/blob/main/Project/bin/lua/zapret-16kb.lua
-- with z2k_-prefix, Lua-version style guards и нашими существующими шимами
-- (apply_fooling / rawsend_dissect / tls_mod_shim / direction_check / etc).
--
-- Идея: ТСПУ обрезает TLS-сессию на ~14-25KB, если соединение классифицировано
-- как блокируемое. Если перед реальным ClientHello отправить N "белых"
-- fake-пакетов (whitelisted SNI, например www.max.ru), DPI может classified
-- соединение как разрешённое и пропустить дальше без 16KB cap. Это и
-- эксплуатируем тремя стратегиями.
-- ----------------------------------------------------------------------------

-- Helper: build white payload (apply tls_mod если задан, чтобы можно было
-- подменять SNI на лету через :tls_mod=sni=www.example.com)
local function z2k_make_white_payload(desync)
    local white = blob(desync, desync.arg.blob)
    if not white then
        error("z2k_16kb: blob required (e.g. blob=tls_max_ru)")
    end
...
-- Helper: split реального payload по pos и отправить сегментами.
-- Возвращает true при успехе, false при rawsend ошибке.
local function z2k_send_real_split(desync, pos, before_each_fn, after_each_fn)
    local data = desync.reasm_data or desync.dis.payload
    if #pos == 0 then
        if before_each_fn then before_each_fn(0) end
        local ok = rawsend_payload_segmented(desync)
        if after_each_fn then after_each_fn(0) end
        return ok ~= false
    end
    for i = 0, #pos do
        ...
        if before_each_fn then before_each_fn(i + 1) end
        if not rawsend_payload_segmented(desync, part, pos_start - 1) then
            return false
        end
        if after_each_fn then after_each_fn(i + 1) end
    end
    return true
end

-- ----------------------------------------------------------------------------
-- z2k_flood_white — отправить N одинаковых белых fake-пакетов перед реальным
-- (multisplit). DPI видит whitelisted handshake'и подряд и помечает flow как
-- разрешённый — последующий 16KB cap не применяется.
--
-- args:
--   blob=<name>        TLS ClientHello blob с белым SNI (default: tls_max_ru)
--   tls_mod=<mods>     модификации white payload (rnd, dupsid, sni=...)
--   count=<N>          количество fake-пакетов (default 10)
--   pos=<positions>    позиции multisplit для реального пакета (default 1,midsld)
--   плюс fooling: badsum / tcp_md5 / ip_ttl / ip_autottl / tcp_seq / tcp_ts
-- ----------------------------------------------------------------------------
function z2k_flood_white(ctx, desync)
    ...
    local white = z2k_make_white_payload(desync)
    local count = tonumber(desync.arg.count) or 10
    if b_debug then DLOG("z2k_flood_white: "..count.." white fakes") end

    -- Шквал одинаковых белых fake'ов
    for i = 1, count do
        local dis = deepcopy(desync.dis)
        dis.payload = white
        apply_fooling(desync, dis)
        if b_debug then DLOG("z2k_flood_white: fake #"..i) end
        rawsend_dissect(dis, {rawsend = {repeats = 1}})
    end

    -- Реальный пакет режем как multisplit
    local data = desync.reasm_data or desync.dis.payload
...
-- ----------------------------------------------------------------------------
-- z2k_white_sandwich — fake-пакеты ДО и ПОСЛЕ каждого сегмента реального
-- пакета. Pattern: white×before — real_seg_1 — white×after — real_seg_2 — ...
-- Гипотеза: ТСПУ retransmit-counter сбивается видя fakes на обеих сторонах.
--
-- args:
--   blob=<name>        white blob (default: tls_max_ru)
--   tls_mod=<mods>     модификации
--   before=<N>         фейков до сегмента (default 3)
--   after=<N>          фейков после сегмента (default 3)
--   pos=<positions>    multisplit позиции (default 1,midsld)
--   плюс fooling
-- ----------------------------------------------------------------------------
function z2k_white_sandwich(ctx, desync)
    if not desync.dis.tcp then
        instance_cutoff_shim(ctx, desync)
        return
    end
    ...
    if b_debug then DLOG("z2k_white_sandwich: before="..before.." after="..after) end

    local function send_fakes(n, label)
        for i = 1, n do
            local dis = deepcopy(desync.dis)
            dis.payload = white
            apply_fooling(desync, dis)
            if b_debug then DLOG("z2k_white_sandwich: "..label.." #"..i) end
            rawsend_dissect(dis, {rawsend = {repeats = 1}})
        end
    end

    local ok = z2k_send_real_split(
        desync, pos,
        function(_) send_fakes(before, "before") end,
        function(_) send_fakes(after,  "after")  end
    )
    if not ok then return VERDICT_PASS end
    replay_drop_set(desync)
    return VERDICT_DROP
end

-- ----------------------------------------------------------------------------
-- z2k_ttl_ladder — fakes с TTL=ttl_min..ttl_max (по одному на каждый),
-- затем реальный multisplit. Брутфорс TTL-расстояния до DPI: при асимметричной
-- маршрутизации / ECMP / cellular / VPN-bastion — `ip_autottl` промахивается,
-- ladder гарантированно один из 10 fake'ов попадёт в нужный хоп.
--
-- args:
--   blob=<name>        white blob (default: tls_max_ru)
--   tls_mod=<mods>     модификации
--   ttl_min=<N>        нижняя граница TTL (default 3)
--   ttl_max=<N>        верхняя граница TTL (default 12)
--   pos=<positions>    multisplit позиции (default 1,midsld)
--   плюс fooling (badsum / tcp_md5 / etc)
-- ----------------------------------------------------------------------------
function z2k_ttl_ladder(ctx, desync)
    if not desync.dis.tcp then
        instance_cutoff_shim(ctx, desync)
        return
    end
    ...
    if ttl_min < 1 then ttl_min = 1 end
    if ttl_max > 64 then ttl_max = 64 end
    if ttl_max < ttl_min then ttl_max = ttl_min end

    if b_debug then DLOG("z2k_ttl_ladder: TTL "..ttl_min..".."..ttl_max) end

    for ttl = ttl_min, ttl_max do
        local dis = deepcopy(desync.dis)
        dis.payload = white
        if desync.dis.ipv6 then dis.ip6_ttl = ttl else dis.ip_ttl = ttl end
        if b_debug then DLOG("z2k_ttl_ladder: ttl="..ttl) end
        rawsend_dissect(dis, {rawsend = {repeats = 1}})
    end
