...
--
-- Один файл, одно имя, РУЧНОЙ рычаг. Штатно имена подбираются на каждую сеть
-- отдельно (state/tcp16_sni.txt, см. z2k_sni_for), и это закрепление —
-- запасной путь: годится, чтобы прижать линию к одному имени руками, когда
-- поимённая карта ещё не собрана или её надо перебить. Здесь только чтение и
-- проверка формы; кто и когда пишет файл — не наше дело.
...
function z2k_sni_pick(ctx, desync)
	...
	-- изменились вовсе.
	--
	-- Поэтому, когда имя для этой сети известно, мы не ведём ни поиска, ни
	-- записи о хосте: подставили и вышли. Карту сетей готовит проба
	-- (z2k-tcp16-probe.sh), перебор по живому трафику выключен и остаётся
	-- запасным путём для линий, где подобрать нечего.
