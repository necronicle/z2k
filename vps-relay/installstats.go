...
var (
	...
	// Отсечка ВКЛЮЧЕНА, но с большим запасом.
	//
	// Механизм был написан и оставлен нулём, то есть выключенным: квоты
	// работали как телеметрия, а не как ограничение. Значение 64 выбрано так,
	// чтобы живой человек его не встретил никогда: замер за три часа дал
	// максимум 2 адреса на установку, порог предупреждения стоит на 12. Даже
	// мобильный интернет со сменой адреса на каждом переподключении и CGNAT
	// не дают такого разброса за сутки.
	//
	// А тот, кто раздаёт туннель дальше, идёт не на единицы и не на десятки, а
	// на сотни адресов — его это ловит. Ноль по-прежнему выключает отсечку,
	// если она вдруг окажется тесной: путь отката должен существовать.
	installMaxIPs = flag.Int("install-max-ips", 64,
